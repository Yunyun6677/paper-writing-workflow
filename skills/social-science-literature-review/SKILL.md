...
For each run, preserve machine-readable records and a UTF-8 LaTeX report. Use [data contract](references/data-contract.md) for handoffs and the repository's `docs/latex-output-standard.md` for publication output. A final review must include search scope, inclusion logic, benchmark-informed writing plan, thematic synthesis, methodological assessment, disagreements, research gaps, ranked innovation points, and a verified bibliography in the requested style.

Every full review must deliver `main.tex`, a separate `innovation-report.tex`, and `references.bib`. It must contain at least one legible figure summarizing the topic's logical structure. Deliver the machine-readable graph, an editable SVG, and a PDF or PNG that `main.tex` can include without shell escape. A prose-only list, word cloud, or decorative mind map does not satisfy this requirement. If rendering or LaTeX compilation fails, the review remains incomplete until repaired or reported as an explicit handoff.
