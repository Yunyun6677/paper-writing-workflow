...
- 四篇公开论文已完成限定范围的真实复现测试：Autor–Dorn–Hanson、Card–Krueger、Acemoglu–Johnson–Robinson 三项指定结果通过，Yang 等（2023）完成表图重建但保留秩亏警告；清单和验证回执见 `tests/fixtures/public-replications/four-paper-benchmark.json` 与 `tests/receipts/v011-four-public-paper-benchmark.json`。这不是全文全部结果复现，也不等于因果识别认证。
