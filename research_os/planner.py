...
def default_graph(paper_type: str) -> list[dict[str, Any]]:
    ...
    nodes = [
        task("frame", "Freeze research question, scope, and contribution", "agent", "research-director", [], expected_outputs=["design/design-register.json"], allowed_tools=["artifact_write"], requires_human_approval=True, success_contract=["research question is explicit", "high-risk choices are surfaced"]),
        ...
        task("literature", "Acquire, verify, and read relevant literature", "agent", "literature-agent", ["question-gate"], expected_outputs=["literature/acquisition-ledger.json", "literature/evidence-cards"], allowed_tools=["bibliographic_lookup", "zotero_local_read", "pdf_parser", "artifact_write"], success_contract=["claims trace to inspected full text"], verification_rules=["citation entailment", "full-text status", "provenance and hashes"]),
        task("literature-review", "Independently verify the literature evidence package", "verifier", "reviewer-verifier-agent", ["literature"], required_inputs={"producer_task_id": "literature"}, allowed_tools=["artifact_write", "evidence_verify"], success_contract=["citation and full-text guardrails pass"], verification_rules=["reviewer differs from producer"]),
    ]
    if empirical:
        nodes += [
            task("empirical-design", "Specify estimand and identification design", "agent", "empirical-agent", ["question-gate", "literature-review"], expected_outputs=["design/design-register.json"], allowed_tools=["artifact_write"], requires_human_approval=True, success_contract=["estimand and identifying assumptions are explicit"]),
            task("design-review", "Independently verify identification and specification", "verifier", "reviewer-verifier-agent", ["empirical-design"], required_inputs={"producer_task_id": "empirical-design"}, allowed_tools=["artifact_write", "evidence_verify"], success_contract=["design diagnostics pass"]),
            ...
            task("analysis", "Run registered analysis and preserve execution receipts", "agent", "empirical-agent", ["design-gate"], expected_outputs=["analysis"], allowed_tools=["python", "stata", "r", "artifact_write"], success_contract=["engine receipts and hashes exist"], verification_rules=["numerical lineage", "specification lock", "reproducibility"]),
            task("empirical-review", "Independently verify empirical artifacts and claims", "verifier", "reviewer-verifier-agent", ["analysis"], required_inputs={"producer_task_id": "analysis"}, allowed_tools=["artifact_write", "evidence_verify"], success_contract=["numeric, causal, and specification guardrails pass"]),
        ]
        writing_dependencies = ["literature-review", "empirical-review"]
    else:
        writing_dependencies = ["literature-review"]
    nodes += [
        task("writing", "Draft the evidence-grounded LaTeX manuscript", "agent", "writing-agent", writing_dependencies, expected_outputs=["paper/main.tex", "paper/references.bib"], allowed_tools=["artifact_write"], success_contract=["citation and numerical firewalls hold"]),
        task("manuscript-review", "Independently review evidence, methods, consistency, privacy, and journal fit", "verifier", "reviewer-verifier-agent", ["writing"], required_inputs={"producer_task_id": "writing"}, expected_outputs=["audit/agent-runtime-audit.json"], allowed_tools=["artifact_write", "evidence_verify"], success_contract=["all blocking guardrails pass"], verification_rules=["citation entailment", "numerical consistency", "research design", "empirical specification", "robustness logic", "manuscript consistency", "reproducibility", "data/privacy", "journal fit"]),
...
def feedback_tasks(request: str, parent_task: str, sequence: int) -> list[dict[str, Any]]:
    ...
    if request == "new_evidence":
        ...
        return [
            {"task_id": evidence, "goal": "Acquire and verify evidence requested by review", "task_type": "agent", "assigned_agent": "literature-agent", "dependencies": [parent_task], "expected_outputs": [f"literature/followup-{suffix}"], "allowed_tools": ["bibliographic_lookup", "zotero_local_read", "pdf_parser", "artifact_write"], "verification_rules": ["full-text status", "citation entailment"]},
            {"task_id": f"manuscript-revision-{suffix}", "goal": "Revise manuscript using verified new evidence", "task_type": "agent", "assigned_agent": "writing-agent", "dependencies": [evidence], "expected_outputs": [f"paper/revision-{suffix}.tex"], "allowed_tools": ["artifact_write"], "verification_rules": ["citation entailment", "manuscript consistency"]},
        ]
    if request == "robustness":
        robustness = f"robustness-followup-{suffix}"
        return [
            {"task_id": robustness, "goal": "Run pre-specified robustness analysis requested by review", "task_type": "agent", "assigned_agent": "empirical-agent", "dependencies": [parent_task], "expected_outputs": [f"analysis/robustness-{suffix}"], "allowed_tools": ["python", "stata", "r", "artifact_write"], "verification_rules": ["specification lock", "numerical lineage", "reproducibility"]},
            {"task_id": f"manuscript-revision-{suffix}", "goal": "Revise manuscript after verified robustness results", "task_type": "agent", "assigned_agent": "writing-agent", "dependencies": [robustness], "expected_outputs": [f"paper/revision-{suffix}.tex"], "allowed_tools": ["artifact_write"], "verification_rules": ["numerical consistency", "manuscript consistency"]},
        ]
